...
async def test_hr_hybrid_and_staff_list(client: AsyncClient, db_session: AsyncSession):
    ...
    admin = User(email="admin_p1@gym.com", hashed_password=hashed, role=Role.ADMIN, full_name="HR Admin Phase1", home_branch_id=branch.id)
    ...
    # 2. Create Employees
    emp1 = User(email="coach_h@gym.com", hashed_password=hashed, role=Role.COACH, full_name="Coach Hybrid", home_branch_id=branch.id)
    emp2 = User(email="cleaner@gym.com", hashed_password=hashed, role=Role.EMPLOYEE, full_name="Cleaner Staff", home_branch_id=branch.id)
    db_session.add_all([emp1, emp2])
    await db_session.flush()
    
    # 3. Create Full-Time Contract for Coach
    contract_data = {
        "user_id": str(emp1.id),
    ...
    # 4. Generate Payroll for Full-Time staff
    ...
    assert data["manual_deductions"] == 0.0
    ...
    assert coach_data["contract"]["type"] == "FULL_TIME"
    assert coach_data["contract"]["commission_rate"] == 0.0
